    ...
    try:
        ...
        for row in rows:
            try:
                ...
                logger.exception(
                    "Conservation outbox publish failed event_id=%s",
                    row.event_id,
                )
        ...
        db.rollback()
        logger.exception("Conservation outbox cycle failed")
...
def run_publisher_loop(interval_seconds: float = 1.0) -> None:
    while True:
        try:
            ...
            logger.exception("Conservation outbox loop error")
...
def start_outbox_publisher() -> None:
    threading.Thread(
        ...
        name="conservation-outbox-publisher",
        daemon=True,
    ).start()